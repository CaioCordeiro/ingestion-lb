        ...
        while retry_count < max_retries:
            try:
                ...
                try:
                    ...
                    self.logger.warning(f"Queue {self.queue_name} exists without dead letter config, declaring normally: {str(e)}")
                    self.channel.queue_declare(
                        queue=self.queue_name,
                        durable=True
                    )
...
class RabbitMQConsumer:
    ...
    def start_consuming(self, callback: Callable[[Dict[str, Any]], bool]) -> None:
        ...
        def process_message(ch, method, properties, body):
            ...
            correlation_id = properties.headers.get('correlation_id', 'unknown') if properties.headers else 'unknown'
